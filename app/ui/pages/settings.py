            ...
            with ui.column().classes('lg:col-span-1 gap-8'):
                ...
                with glass_card(classes='p-6'):
                    with ui.row().classes('items-center gap-3 mb-4'):
                        ui.icon('rss_feed').classes('text-orange-400 text-xl')
                    ...
                    with ui.column().classes('gap-3 w-full'):
                        ...
                        with ui.row().classes('w-full gap-2'):
                            ui.button('Copy', on_click=lambda: ui.run_javascript(f'navigator.clipboard.writeText("{feed_url}")')).props('flat dense no-caps').classes('flex-1 bg-white/10 hover:bg-white/20 text-white border border-white/10 rounded-lg transition-colors text-sm')
                            ui.button('Open', on_click=lambda: ui.run_javascript(f'window.open("{feed_url}", "_blank")')).props('flat dense no-caps').classes('flex-1 bg-orange-500/20 hover:bg-orange-500/30 text-orange-300 border border-orange-500/30 rounded-lg transition-colors text-sm')

                # 2. API 配置
                with glass_card(classes='p-6'):
                    with ui.row().classes('items-center gap-3 mb-4'):
                        ui.icon('vpn_key').classes('text-purple-400 text-xl')
                    ...
                    with ui.column().classes('gap-4 w-full'):
                        ...
                        ui.button('Save Key', on_click=save_api_key).props('unelevated no-caps').classes('w-full bg-purple-600 hover:bg-purple-500 text-white shadow-[0_0_15px_rgba(147,51,234,0.4)] rounded-lg font-medium')

                # 3. 浏览器控制
                with glass_card(classes='p-6'):
                    with ui.row().classes('items-center gap-3 mb-4'):
                        ui.icon('public').classes('text-cyan-400 text-xl')
                        ui.label('Browser Control').classes('text-lg font-bold text-white')
                    
                    from app.services.scraper_service import open_login_browser
                    ui.button('Launch Login Browser', on_click=open_login_browser, icon='login').props('unelevated no-caps').classes('w-full bg-cyan-600/80 hover:bg-cyan-500 text-white border border-cyan-400/30 rounded-lg')
